...
if [[ ${AV_TEST_GITHUB_OAUTH:-0} == 1 ]]; then
  ...
  bao kv get -field=GITHUB_CLIENT_ID "$openbao_secret_path" > "$temporary_secret_directory/client-id"
  bao kv get -field=GITHUB_CLIENT_SECRET "$openbao_secret_path" > "$temporary_secret_directory/client-secret"
